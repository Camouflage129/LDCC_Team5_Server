...
public class ProductController {
	...
	@CrossOrigin
	@RequestMapping(value="/getProductInfo/{num}", method = RequestMethod.GET)
	...
	@CrossOrigin
	@RequestMapping(value="/deliveryComplete/{code}", method = RequestMethod.GET)
	public ResponseEntity<String> deliveryComplete(@PathVariable("code") String code) {
		ResponseEntity<String> resEntity = null;
		try {
			int row = service.deliveryComplete(code);
			if(row > 0)
	...
	public ResponseEntity<List<Integer>> userStatus(@PathVariable("id") String id, @PathVariable("date") Date date) {
		...
		try {
			...
			int remain = service.getRemainBox();
			int total = service.getTotalBox(new ProductInfoModel(id, date));
			if(total > 0) {
